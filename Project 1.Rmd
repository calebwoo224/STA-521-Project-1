...
data_all$VPD = 6.1078*exp((17.269*data_all$humid_temp)/(237.3+data_all$humid_temp))*(1-(data_all$humid_adj/100))
X = log(data_all$humid_adj/100) + ((17.269*data_all$humid_temp)/(237.3+data_all$humid_temp))
data_all$dewpoint = (237.3*X)/(17.269-X)
...
data_all_sun <- data_all_sun %>%
  mutate(Time = case_when(
    ...
    epochDates %within% interval(`End Sunrise`, `Solar Noon`) ~ "Morning",
    epochDates %within% interval(`Solar Noon`, `Begin Sunset`) ~ "Afternoon",
    T ~ "Night"
  ))
data_all_sun <- data_all_sun %>%
  mutate(Hour = hour(epochDates))
data_all_sun$Time <- factor(data_all_sun$Time, levels = c("Night", "Sunrise", "Morning", "Afternoon", "Sunset"))
```
...
### Afternoon vs Night Clustering

```{r afternoon-night-plot}
data_all_sun %>%
  filter(Time == "Afternoon" | Time == "Night") %>%
  ggplot(., aes(x=humid_temp, y=humid_adj, color=Time)) +
  geom_point()
```

```{r afternoon-night-clustering}
library(mclust)
set.seed(521)

after_night_gmm <- data_all_sun %>%
  filter(Time == "Afternoon" | Time == "Night") %>%
  select(humid_temp, humid_adj) %>%
  Mclust(., G=2, modelNames="EEE", prior=priorControl())

after_night_clusters_data <- data_all_sun %>%
  filter(Time == "Afternoon" | Time == "Night") %>%
  select(humid_temp, humid_adj) %>%
  cbind(., after_night_gmm$classification)

names(after_night_clusters_data)[3] <- "Cluster"
after_night_clusters_data$Cluster <- as.factor(after_night_clusters_data$Cluster)

ggplot(after_night_clusters_data, aes(x=humid_temp, y=humid_adj, color=Cluster)) +
  geom_point()
```

### Sunrise vs Sunset Clustering

```{r sunrise-sunset-plots}
data_all_sun %>%
...
```{r sunrise-sunset-clustering}
library(mclust)
set.seed(521)

rise_set_gmm <- data_all_sun %>%
  filter(Time == "Sunrise" | Time == "Sunset") %>%
  select(humid_temp, humid_adj) %>%
  Mclust(., G=2, modelNames="EVI", prior=priorControl())

rise_set_clusters_data <- data_all_sun %>%
  filter(Time == "Sunrise" | Time == "Sunset") %>%
  select(humid_temp, humid_adj) %>%
  cbind(., rise_set_gmm$classification)

names(rise_set_clusters_data)[3] <- "Cluster"
rise_set_clusters_data$Cluster <- as.factor(rise_set_clusters_data$Cluster)

ggplot(rise_set_clusters_data, aes(x=humid_temp, y=humid_adj, color=Cluster)) +
  geom_point()
```
