...
data_all_sun$Time <- factor(data_all_sun$Time, levels = c("Early Night", "Late Night", "Sunrise", "Early Morning",
                                                          "Late Morning", "Early Noon", "Late Noon", "Sunset"))
```

```{r may-data-sun}
data_all_sun <- data_all_sun %>%
  select(-c("...1", "...2", "Sunrise", "Sunset", "Solar Noon", "Begin Sunrise",
            "End Sunset", "End Sunrise", "Begin Sunset", "Time", "Hour"))
...
### Early Noon vs Late Night Clustering

```{r plot1}
data_all_sun %>%
  filter(Time == "Early Noon" | Time == "Late Night") %>%
...
early_late_gmm <- data_all_sun %>%
  filter(Time == "Early Noon" | Time == "Late Night") %>%
  select(humid_adj, humid_temp) %>%
  Mclust(., G=2, modelNames="VVV", prior=priorControl())

early_late_clusters_data <- data_all_sun %>%
  filter(Time == "Early Noon" | Time == "Late Night") %>%
  select(humid_adj, humid_temp) %>%
  cbind(., early_late_gmm$classification)

names(early_late_clusters_data)[3] <- "Cluster"
early_late_clusters_data$Cluster <- as.factor(early_late_clusters_data$Cluster)

ggplot(early_late_clusters_data, aes(x=humid_adj, y=humid_temp, color=Cluster)) +
  geom_point()
```

```{r plot2}
data_all_sun %>%
  filter(Time == "Early Noon" | Time == "Early Morning") %>%
  ggplot(., aes(x=hamatop, y=humid_temp, color=Time)) +
  geom_point()
```
...
early_both_gmm <- data_all_sun %>%
  filter(Time == "Early Noon" | Time == "Early Morning") %>%
  select(hamatop, humid_temp) %>%
  Mclust(., G=2, modelNames="VVV", prior=priorControl())

early_both_clusters_data <- data_all_sun %>%
  filter(Time == "Early Noon" | Time == "Early Morning") %>%
  select(hamatop, humid_temp) %>%
  cbind(., early_both_gmm$classification)

names(early_both_clusters_data)[3] <- "Cluster"
early_both_clusters_data$Cluster <- as.factor(early_both_clusters_data$Cluster)

ggplot(early_both_clusters_data, aes(x=hamatop, y=humid_temp, color=Cluster)) +
  geom_point()
```
